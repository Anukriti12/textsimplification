...
const Review = () => {
  ...
  /* UI‑state */
  const [diffHtml, setDiffHtml] = useState("");
  const [isSaveButtonEnabled, setIsSaveButtonEnabled] = useState(true); // CHANGED: Default to true
  const [isEditable, setIsEditable] = useState(false);
  const [isSidebarVisible, setIsSidebarVisible] = useState(false);
  const [showDifference, setShowDifference] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
...
const generateDiff = (a = "", b = "") => {
  ...
  const handleEditChange = (e) => {
    const txt = e.target.value;
    setOutputText(txt);
    // REMOVED: setIsSaveButtonEnabled(true); - No longer needed since it's always enabled
    saveEditToHistory(txt);
  };
  ...
  const saveFinalOutput = async () => {
    ...
    try {
      ...
      const res = await fetch("https://textsimplification12-a0a8gqfbhnhxbgbv.westus-01.azurewebsites.net/api/simplifications/final", {
       ...
       if (res.ok) {
                /* keep UI changes … */
                // REMOVED: setIsSaveButtonEnabled(false); - Keep button enabled
                setShowSurveyPrompt(true);
                setSaveHistory((p) => [...p, { timestamp: new Date().toISOString(), finalText: outputText }]);
                setTimeout(() => surveyRef.current?.scrollIntoView({ behavior: "smooth" }), 300);
        ...
        <div className={`${styles.mainContent} ${isSidebarVisible ? styles.withSidebar : ""}`}>
          ...
          <div className={styles.button_container}>
            <button
              className={styles.submit_btn}
              onClick={saveFinalOutput}
              disabled={isLoading} // CHANGED: Only disabled during loading, not based on edits
              // REMOVED: title tooltip that mentioned needing to edit first
            >
              Save
            </button>
          </div>
